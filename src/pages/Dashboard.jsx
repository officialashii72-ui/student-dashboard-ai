...
import { getGuestSampleTasks, getGuestSampleNotes, getGuestSampleSubjects } from '../services/guestDataHelpers';
 ...
 * Dashboard Component
 * 
 * Shows user's academic progress with tasks, notes, subjects, and study plan
 * Supports both authenticated and guest modes:
 * - Authenticated: Loads real data from Firestore
 * - Guest: Shows sample data to demonstrate functionality
 */
const Dashboard = () => {
    ...
    const fetchAllData = useCallback(async () => {
        ...
        try {
            ...
            // For guest users: load sample data
            else if (isGuest) {
                setTasks(getGuestSampleTasks());
                setNotes(getGuestSampleNotes());
                setSubjects(getGuestSampleSubjects());
            }
        } catch (error) {
                ...
                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
                    <div className="max-w-xl">
                        <span className="inline-block px-4 py-1.5 bg-blue-600/20 border border-blue-500/30 rounded-full text-blue-300 text-xs font-bold uppercase tracking-widest mb-4">
                            {isGuest ? 'Guest Mode Preview' : 'Student Dashboard v2.0'}
                        </span>
                        ...
                        <p className="text-slate-300 text-base sm:text-lg leading-relaxed">
                            {isGuest 
                                ? "This is a preview of your dashboard. Sign up to save your real tasks, notes, and unlock full AI features!"
                                : "Track your progress, manage notes, and plan your study sessions with our Firestore-powered dashboard."
                            }
                        </p>
                    </div>
